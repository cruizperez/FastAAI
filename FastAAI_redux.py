...
def convert_float_array(bytestring):
	return np.frombuffer(bytestring, dtype = glob_prec)
...
class calculation_database:
	...
	def activate_connection(self):
		...
		# Converts TEXT to np.array when selecting
		sqlite3.register_converter("array", convert_float_array)
...
class input_file:
	...
	def preprocess(self):
		...
		if self.status == 'genome':
			...
			start_time = timer.strftime(time_format)
			#report = True
		...
		if self.start_time is not None:
			timer = datetime.datetime.now()
			end_time = timer.strftime(time_format)
...
def advance_inputs(genomes = None, proteins = None, hmms = None, genomes_file = None, proteins_file = None, hmms_file = None, output = "FastAAI", threads = 1, verbose = False, db_name = ""):
	...
	#progress bar - possible dangerous use of the return to line start sequence.
		try:
			percentage = 0
			print("Completion".rjust(3)+ ' |'+('#'*int(percentage/2)).ljust(50)+'| ' + ('%.2f'%percentage).rjust(7)+'% (Genome ' + str(count) + " of " + str(total_counts) + ')')
		except:
	...
	time_format = "%d/%m/%Y %H:%M:%S"
	timer = datetime.datetime.now()
	log_time = timer.strftime(time_format)
...
def add_inputs(output_path, parent_path, existing_index, threads, verbose, prep_args):
	...
	if verbose:
		...
		try:
			...
			log_time = timer.strftime(time_format)
			percentage = (count/total_counts)*100
			print("Completion".rjust(3)+ ' |'+('#'*int(percentage/2)).ljust(50)+'| ' + ('%.2f'%percentage).rjust(7)+'% ( ' + str(count) + " of " + str(total_counts) + ' )')
		except:
			#It's not really a big deal if the progress bar cannot be printed.
			pass
	...
	for result in pool.imap_unordered(produce_children, child_args):
		...
		if verbose:
			...
			try:
				...
				sys.stdout.write('\033[A')
				sys.stdout.flush()
				print("Completion".rjust(3)+ ' |'+('#'*int(percentage/2)).ljust(50)+'| ' + ('%.2f'%percentage).rjust(7)+'% ( ' + str(count) + " of " + str(total_counts) + ' ) done at '+ log_time)
			except:
				#It's not really a big deal if the progress bar cannot be printed.
				pass
...
def do_query_vs_target_aai_only(query_name, target_name, threads, output, precision, verbose):
	...
	rdb = calculation_database(rdb_name)
	...
	query_args = [([rdb_name], query_chunks[i], output, query, target) for i in range(0, threads)]
...
def finish_jaccards(args):
	partial_dbs, my_query_genomes, output, query, target = args[0], args[1], args[2], args[3] ,args[4]
	#Load protein counts
	...
	for db in partial_dbs:
		activated_DBs.append(calculation_database(db))
...
def do_query_vs_target_sql(query, target, threads, output, verbose, do_stdev):
	...
	print("")
	print("FastAAI will search", query_length, "query genomes against", target_length, "target genomes.")
	print("")
	...
	#progress bar - possible dangerous use of the return to line start sequence.
		try:
			percentage = 0
			print("Completion".rjust(3)+ ' |'+('#'*int(percentage/2)).ljust(50)+'| ' + ('%.2f'%percentage).rjust(7)+'% (Query genome ' + str(count) + " of " + str(total) + ' done at '+curtime()+')')
			last_pct = 0
		except:
			#It's not really a big deal if the progress bar cannot be printed.
			pass
	...
	#If not parallelized, don't bother setting up the parallel process.
	else:
		for file in query_args:
			
			if do_stdev:
				result = do_sql_query(file)
			else:
				result = do_sql_query_no_SD(file)
...
def merge_db(recipient, donors, verbose, threads):
	...
	if recip_check == "exists":
		...
		ct = 0
		path = recipient.path
		reverse_genome_indicies[path] = []
		for idx in sorted(recipient.genome_index.values()):
			reverse_genome_indicies[path].append(idx)
		reverse_genome_indicies[path] = np.array(reverse_genome_indicies[path], dtype = np.int32)
	...
	for d in donor_dbs:
		...
		d.accessions = None
		reverse_genome_indicies[d.path] = []
		...
		for g in sorted(d.genome_index.keys()):
			if g not in joint_genome_index:
				reverse_genome_indicies[d.path].append(max_index)
				...
				reverse_genome_indicies[d.path].append(joint_genome_index[g])
		#Make it an array, now
		reverse_genome_indicies[d.path] = np.array(reverse_genome_indicies[d.path], dtype = np.int32)
		d.genome_index = None
...
def main():
	...
	if selection not in modules:
		...
		print("---------------------------------------------- Query Options ----------------------------------------------")
		print("")
		print("    sql_query |" + " Query a genome or protein (one or many) against an existing FastAAI database")
		print("    db_query     |" + " Query the genomes in one FastAAI database against another FastAAI database")
		print("")
		print("-----------------------------------------------------------------------------------------------------------")
		print("")
		print("    To select a module, enter 'FastAAI [module]' into the command line!")
