...
# # --- Parse kAAI ---
# def kAAI_Parser2(input_data):
#     print("PARSER {}".format(datetime.datetime.now())) # Remove after testing
#     genome_id = input_data[0]
#     kmer_dict = input_data[1]
#     file_path = Path(genome_id)
#     running_folder = Path.cwd()
#     temp_output = running_folder / file_path.with_suffix('.aai.temp')
#     query_num_scg = len(kmer_dict[genome_id])
#     query_scg_list = kmer_dict[genome_id].keys()
#     with open(temp_output, 'w') as out_file:
#         for target_genome, scg_ids in kmer_dict.items():
#             print(target_genome)
#             jaccard_similarities = []
#             target_num_scg = len(scg_ids)
#             target_scg_list = scg_ids.keys()
...
#             for accession in final_scg_list:
#                 if accession in query_scg_list and accession in target_scg_list:
#                     kmers_query = kmer_dict[genome_id][accession]
#                     kmers_target = kmer_dict[target_genome][accession]
#                     intersection = len(kmers_query.intersection(kmers_target))
#                     union = len(kmers_query.union(kmers_target))
#                     jaccard_similarities.append(intersection / union)
#                 else:
#                     continue
#             try:
#                 out_file.write("{}\t{}\t{}\t{}\t{}\n".format(genome_id, target_genome,
#                            sum(jaccard_similarities)/len(jaccard_similarities),
#                            len(jaccard_similarities), len(final_scg_list)))
#             except:
#                 out_file.write("{}\t{}\t{}\t{}\t{}\n".format(genome_id, target_genome,
#                            "NA", "NA", "NA"))
#     print("ENDED {}".format(datetime.datetime.now())) # Remove after testing
#     return temp_output

class Class_Kmer_Dict():
    shared_kmer_dict = None

    @classmethod
    def merge_dicts(cls, dict_list):
        """
        ...
        cls.shared_kmer_dict = result.copy()

    @classmethod
    def kAAI_Parser2(cls, query_id):
        # print("PARSER {}".format(datetime.datetime.now())) # Remove after testing
        file_path = Path(query_id)
        running_folder = Path.cwd()
        temp_output = running_folder / file_path.with_suffix('.aai.temp')
        query_num_scg = len(cls.shared_kmer_dict[query_id])
        query_scg_list = cls.shared_kmer_dict[query_id].keys()
        with open(temp_output, 'w') as out_file:
            for target_genome, scg_ids in cls.shared_kmer_dict.items():
                # print(target_genome)
                jaccard_similarities = []
                target_num_scg = len(scg_ids)
                target_scg_list = scg_ids.keys()
                ...
                for accession in final_scg_list:
                    if accession in query_scg_list and accession in target_scg_list:
                        kmers_query = set(cls.shared_kmer_dict[query_id][accession])
                        kmers_target = set(cls.shared_kmer_dict[target_genome][accession])
                        intersection = len(kmers_query.intersection(kmers_target))
                        union = len(kmers_query.union(kmers_target))
                        jaccard_similarities.append(intersection / union)
                    else:
...
def main():
    ...
    # final_kmer_dictionary = manager.dict()
    ...
    # print(sys.getsizeof(tuple(Kmer_Results[0]['GCA_000005845']['PF06026.14'])))
    # print(Kmer_Results[0]['GCA_000005845']['PF06026.14'])
    ###========= Create class ================= PF06026.14
    kmer_dict_information = Class_Kmer_Dict()
    # print(kmer_dict_information)
    kmer_dict_information.merge_dicts(Kmer_Results)
    # print(kmer_dict_information.shared_kmer_dict)


    # Final_Kmer_Dict = merge_dicts(Kmer_Results)
    # final_kmer_dictionary.update(Final_Kmer_Dict)
    # Final_Kmer_Dict = None

    # Calculate shared Kmer fraction
    print("Calculating shared Kmer fraction...")
    print(datetime.datetime.now())
    ID_List = kmer_dict_information.shared_kmer_dict.keys()
    elements = len(ID_List)
    try:
        pool = multiprocessing.Pool(Threads)
        # pool = multiprocessing.Pool(Threads, initializer = child_initialize, initargs = (Final_Kmer_Dict,))
        Fraction_Results = pool.map(kmer_dict_information.kAAI_Parser2, ID_List)
        # Fraction_Results = pool.map(kAAI_Parser2, zip(ID_List, repeat(final_kmer_dictionary))) 
        # Fraction_Results = pool.starmap(kAAI_Parser, (ID_List, repeat(final_kmer_dictionary, elements)))
    finally:
        pool.close()
        pool.join()
